...
const formatTimestamp = (eventIndex: number, baseDate = '2026-04-06'): string => {
...
const findTask = (tasks: TaskInfo[], taskId: number): TaskInfo => {
...
describe('Strict task projector semantics', () => {
  it('does not synthesize fallback action from pipeline terminal action_details', async () => {
    const lines = [
      makeEventLine(
        ...
        { task_id: 81, entry: 'MainTask', hash: 'h-81', uuid: 'u-81' },
        '2026-04-11',
      ),
      makeEventLine(
        2,
        'Node.PipelineNode.Starting',
        { task_id: 81, node_id: 8101, name: 'MainNode' },
        '2026-04-11',
      ),
      makeEventLine(
        3,
        'Node.PipelineNode.Failed',
        {
          ...
          node_id: 8101,
          name: 'MainNode',
          action_details: makeActionDetails({
            actionId: 9001,
            name: 'FallbackAction',
            success: false,
          }),
  ...
  it('does not synthesize fallback action when pipeline terminal action_details is absent', async () => {
    const lines = [
      makeEventLine(
        11,
        'Tasker.Task.Starting',
        { task_id: 82, entry: 'MainTask', hash: 'h-82', uuid: 'u-82' },
        '2026-04-11',
      ),
      makeEventLine(
        12,
        'Node.PipelineNode.Starting',
        { task_id: 82, node_id: 8201, name: 'MainNode' },
        '2026-04-11',
      ),
      makeEventLine(
        13,
  ...
  it('keeps main wait_freezes as a top-level sibling instead of nesting it into subtask flow', async () => {
    ...
    const topLevelMainWaitFreezes = (mainNode.node_flow ?? []).filter(
      (item) => item.type === 'wait_freezes' && item.task_id === 1,
    )
    expect(topLevelMainWaitFreezes).toHaveLength(1)

    const nestedMainWaitFreezes = collectFlowItems(
      mainNode.node_flow,
      (item, path) => item.type === 'wait_freezes' && item.task_id === 1 && path.length > 1,
    )
  ...
  it('does not synthesize running action placeholder before explicit action scope exists', () => {
    ...
    const lines = [
      ...
      makeEventLine(101, 'Node.PipelineNode.Starting', {
        task_id: 200000021,
        node_id: 300000021,
        name: 'test',
      }),
      makeEventLine(102, 'Node.Recognition.Starting', {
        task_id: 200000021,
        reco_id: 400000021,
        name: 'test',
      }),
      ...
      makeEventLine(105, 'Node.Recognition.Starting', {
        task_id: 200000022,
        reco_id: 400000022,
        name: 'Stop',
      }),
      makeEventLine(106, 'Node.Recognition.Succeeded', {
        task_id: 200000022,
        reco_id: 400000022,
        name: 'Stop',
      }),
      makeEventLine(107, 'Node.Action.Starting', {
        task_id: 200000022,
        action_id: 500000021,
        name: 'Stop',
      }),
      ...
      makeEventLine(110, 'Tasker.Task.Succeeded', {
        task_id: 200000022,
        entry: 'Stop',
        hash: 'h-sub-a',
        uuid: 'u-sub-a',
  ...
  it('projects taskless context child scopes under recognition and action windows', async () => {
    const lines = [
      ...
      makeEventLine(4, 'Node.Recognition.Starting', {
        task_id: 1,
        reco_id: 501,
        name: 'CCBuyCard',
      }),
      makeEventLine(5, 'Node.RecognitionNode.Starting', {
        task_id: 2,
        node_id: 201,
        name: 'CCBuyCardAwardEmptyRec',
      }),
      makeEventLine(6, 'Node.Recognition.Starting', {
        task_id: 2,
        reco_id: 601,
        name: 'CCBuyCardAwardEmptyRec',
      }),
      makeEventLine(7, 'Node.RecognitionNode.Starting', {
        task_id: 3,
        node_id: 301,
        name: 'CCBuyCardAwardEmptyRec_Template',
      }),
      makeEventLine(8, 'Node.Recognition.Starting', {
        task_id: 3,
        reco_id: 701,
        name: 'CCBuyCardAwardEmptyRec_Template',
      }),
      ...
      makeEventLine(16, 'Node.PipelineNode.Starting', {
        task_id: 10,
        node_id: 1001,
        name: 'CCUpdate',
      }),
      ...
      makeEventLine(18, 'Node.Recognition.Starting', {
        task_id: 10,
        reco_id: 1101,
        name: 'CCUpdate',
      }),
      ...
      makeEventLine(26, 'Tasker.Task.Succeeded', {
        task_id: 1,
        entry: 'MainTask',
        hash: 'h-main',
        uuid: 'u-main',
    ...
    expect(nestedRecognitionNode?.children?.[0]?.children?.map((item) => item.type)).toEqual([
      'recognition_node',
    ])
  ...
  it('keeps CCBuyCard nested recognition/action structure from the CCFlagInCombatMain fixture', async () => {
    ...
    expect(mainRecognition?.children?.[0]?.children?.[0]?.children?.[0]?.task_id).toBe(200001554)
  ...
  it('deduplicates mirrored cross-source recognition starts before they can capture later flow', async () => {
    ...
    const lines = [
      makeEventLine(
        100,
        'Tasker.Task.Starting',
        { task_id: 401, entry: 'StartUp', hash: 'h-main', uuid: 'u-main' },
        '2026-04-12',
      ),
      makeEventLine(
        101,
        'Node.PipelineNode.Starting',
        { task_id: 401, node_id: 4101, name: 'StartUp' },
        '2026-04-12',
      ),
      makeEventLine(
        102,
      ...
      makeEventLine(
        ...
        { task_id: 401, reco_id: 5101, name: 'StartUp' },
        '2026-04-12',
        {
          processId: 'Px1',
          threadId: 'Tx1',
      ...
      makeEventLine(
        ...
        { task_id: 401, action_id: 6101, name: 'StartUp' },
        '2026-04-12',
      ),
      makeEventLine(
        119,
    ...
    expect(
      startUpAction[0]?.path.some(
        (pathNode) => pathNode.type === 'recognition' && pathNode.reco_id === 5101,
      ),
    ).toBe(false)
  })
  ...
  it('does not keep a duplicate recognition start running after parser dedup keeps only one terminal', async () => {
    ...
    const lines = [
      ...
      makeEventLine(
        103,
        'Node.Recognition.Starting',
        { task_id: 301, reco_id: 4101, name: 'StartUp' },
        '2026-04-06',
        {
          processId: 'Px1',
          threadId: 'Tx1',
      ...
      makeEventLine(125, 'Node.Action.Starting', {
        task_id: 301,
        action_id: 5101,
        name: 'StartUp',
      }),
      makeEventLine(126, 'Tasker.Task.Starting', {
        task_id: 302,
        entry: 'ChildTask',
        hash: 'h-child',
        uuid: 'u-child',
      }),
      makeEventLine(127, 'Node.PipelineNode.Starting', {
        task_id: 302,
        node_id: 3201,
        name: 'ChildNode',
      }),
      makeEventLine(128, 'Node.PipelineNode.Succeeded', {
        task_id: 302,
        node_id: 3201,
        name: 'ChildNode',
      }),
      makeEventLine(129, 'Tasker.Task.Succeeded', {
        task_id: 302,
        entry: 'ChildTask',
        hash: 'h-child',
        uuid: 'u-child',
      ...
      makeEventLine(131, 'Node.PipelineNode.Succeeded', {
        task_id: 301,
        node_id: 3101,
        name: 'StartUp',
      }),
      makeEventLine(132, 'Tasker.Task.Succeeded', {
        task_id: 301,
        entry: 'StartUp',
        hash: 'h-main',
        uuid: 'u-main',
    ...
    const runningStartUpRecognitions = collectFlowItems(
      mainNode.node_flow,
      (item) => item.type === 'recognition' && item.reco_id === 4101 && item.status === 'running',
    )
    ...
    expect(
      childTaskItems[0]?.path.some(
        (pathNode) => pathNode.type === 'recognition' && pathNode.reco_id === 4101,
      ),
    ).toBe(false)
    expect(
      childTaskItems[0]?.path.some(
        (pathNode) => pathNode.type === 'action' && pathNode.action_id === 5101,
      ),
    ).toBe(true)
  })
  ...
  it('ignores delayed cross-source mirrored recognition starts while the primary scope is still open', async () => {
    ...
    const lines = [
      ...
      makeEventLine(
        2735,
        'Node.Action.Starting',
        { task_id: 401, action_id: 6101, name: 'ClickTarget' },
        '2026-04-16',
      ),
      makeEventLine(
        2789,
      ...
      makeEventLine(
        5000,
        'Tasker.Task.Starting',
        { task_id: 402, entry: 'ChildTask', hash: 'h-child', uuid: 'u-child' },
        '2026-04-16',
      ),
      makeEventLine(
        5001,
        'Node.PipelineNode.Starting',
        { task_id: 402, node_id: 4201, name: 'ChildNode' },
        '2026-04-16',
      ),
      makeEventLine(
        5002,
    ...
    const runningAlreadyThere = collectFlowItems(
      mainNode.node_flow,
      (item) => item.type === 'recognition' && item.reco_id === 5101 && item.status === 'running',
    )
  ...
  it('projects focus onto node, recognition, and action scopes without synthetic focus context', async () => {
    ...
    const lines = [
      makeEventLine(
        1,
        'Tasker.Task.Starting',
        { task_id: 71, entry: 'FocusTask', hash: 'h-71', uuid: 'u-71' },
        '2026-04-12',
      ),
      makeEventLine(
        2,
  ...
  it('does not expose NaN durations for invalid or reversed timestamps', async () => {
    ...
    const parser = new LogParser()
    await parser.parseFile([invalidStart, invalidEnd, reversedStart, reversedEnd].join('\n'))
