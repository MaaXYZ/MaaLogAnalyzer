...
import { buildActionDetailRows, buildRecognitionDetailRows, formatDetailValue } from './detailRows'
...
describe('detailRows', () => {
    ...
    const rows = buildRecognitionDetailRows(
      {
        ...
        detail: {
          ...
          filtered: [{ box: [10, 20, 30, 40], score: 0.92, text: 'OK' }],
  ...
  it('builds And/Or recognition child summaries', () => {
    ...
    expect(rows[3].value).toBe(
      '命中 Icon (TemplateMatch)，box=[1, 2, 3, 4]，all 1，filtered 1，best score=0.9',
    )
  })
  ...
  it('builds shell and screencap rows from MaaFramework runtime detail', () => {
    ...
    expect(shellRows.map((row) => row.label)).toEqual([
      '命令',
      'Shell 超时',
      '详情结果',
      'Shell 输出',
    ...
    expect(screencapRows.map((row) => row.label)).toEqual([
      '截图路径',
      '截图格式',
      '截图质量',
      '详情结果',
