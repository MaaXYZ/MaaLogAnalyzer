    ...
    <div style="padding: 8px 12px; border-bottom: 1px solid var(--n-border-color)">
      <n-flex align="center" justify="space-between">
        <n-text depth="3" style="font-size: 12px">
          显示行 {{ props.contextStartLine }} -
          {{ props.contextStartLine + props.contextLines.length - 1 }} （共
          {{ props.contextLines.length }} 行）
        </n-text>
      ...
      >
        <div
          ...
          :style="{
            padding: '2px 8px',
            backgroundColor:
              props.contextStartLine + idx === props.selectedLine
                ? 'var(--n-color-target)'
                : 'transparent',
            borderRadius: '2px',
          }"
